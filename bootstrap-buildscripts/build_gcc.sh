...
sed -i 's/PATH_MAX/256/' libsanitizer/asan/asan_linux.cc

cd build-target

../configure --host=$TARGET --target=$TARGET --prefix=/usr --enable-languages=c,c++ --disable-libstdcxx-pch --disable-multilib \
...
ln -s /usr/bin/cpp /new_root/lib/cpp 
