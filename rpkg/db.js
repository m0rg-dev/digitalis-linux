...
exports.PackageDescription = exports.Database = void 0;
...
class Database {
            ...
            fs.readdir(path.join(this.database_path, atom.getCategory()), function (err, files) {
                if (err) {
                    ...
                    res(versions.filter(version => semver.satisfies(version, atom.getVersions()))
                        .map(version => new atom_js_1.SingleVersionAtom(atom.getCategory(), atom.getName(), version)));
                }
            });
        });
    }
    async getPackageDescription(atom) {
        return new Promise((res, rej) => {
            fs.readFile(path.join(this.database_path, atom.getCategory(), atom.getName() + "@" + atom.getVersion().version + ".yml"), function (err, data) {
                if (err) {
                    rej(err);
                }
                else {
