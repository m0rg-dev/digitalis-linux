...
class Repository {
    ...
    async buildPackage(atom, container_id, container_mountpoint) {
        ...
        tar.on('exit', (status, signal) => {
            if (signal)
                throw `tar killed by signal ${signal}`;
            if (status != 0)
