...
async function main() {
    ...
    const target_dir: string = process.argv[5] || path.join(process.argv[2], "builds");
    ...
    try {
        ...
        if (pkgdesc.pre_configure_script) run_build_stage(container_id, "pre-configure script", pkgdesc.pre_configure_script);
        if (pkgdesc.configure) run_build_stage(container_id, "configure", pkgdesc.configure);
        if (pkgdesc.make) run_build_stage(container_id, "make", pkgdesc.make);
        if (pkgdesc.install) run_build_stage(container_id, "make install", pkgdesc.install);

        child_process.spawnSync('buildah', ['config', '--workingdir', '/target_root', container_id]);

        if (pkgdesc.post_install_script) run_build_stage(container_id, "post-install script", pkgdesc.post_install_script);
        // TODO autodetect
        if (pkgdesc.queue_hooks['ldconfig']) run_build_stage(container_id, "ldconfig", "ldconfig -N -r .");
...
function run_build_stage(container_id: string, name: string, script: string) {
