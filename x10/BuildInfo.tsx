            ...
            {completion => {
                if (completion.failed() == FailureType.not_built) {
                    return <ink.Box key={completion.hash()}>
                        <ink.Text><ink.Text color="yellow">?</ink.Text> Not built: {path.basename(completion.spec.spec)} ({completion.name}) for {completion.installed_on}</ink.Text>
                    </ink.Box>;
                } else if (completion.failed() == FailureType.failed) {
                    return <ink.Box key={completion.hash()}>
                        <ink.Text><ink.Text color="red">✘</ink.Text> Failed: {path.basename(completion.spec.spec)} ({completion.name}) for {completion.installed_on}</ink.Text>
                    </ink.Box>;
                } else {
                    return <ink.Box key={completion.hash()}>
                        <ink.Text><ink.Text color="green">✔</ink.Text> Built: {path.basename(completion.spec.spec)} ({completion.name}) for {completion.installed_on}</ink.Text>
                    </ink.Box>;
                }
            }}
        </ink.Static>;
